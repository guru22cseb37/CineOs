...
export default function MovieCard({ movie }: MovieCardProps) {
  ...
  // Fetch trailer only once per card
      ...
      >
        ...
        <div className={`absolute inset-0 bg-black/80 ${showTrailer && trailerKey ? 'opacity-0' : 'opacity-0 group-hover:opacity-100'} transition-opacity duration-300 flex flex-col justify-end p-4 z-20`}>
          ...
          <div className="flex items-center mt-2 space-x-2">
            <div className="w-8 h-8 rounded-full bg-white flex items-center justify-center text-black hover:bg-cinema-gold transition-colors">
              <Play className="w-4 h-4 fill-current" />
            </div>
            <div className="w-8 h-8 rounded-full border border-white/30 flex items-center justify-center text-white hover:border-white transition-colors">
